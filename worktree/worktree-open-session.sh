...
# --remote-control '<slug>' so the session shows up nicely named in Remote Control.
#
...
open_tmux_fallback() {
  ...
  tmux new-session -d -s "$tname" -c "$wt" \
    "$HOME/.config/harness/worktree-setup.sh '${branch}'; claude --remote-control '${tname}'" \
...
cmd="~/.config/harness/worktree-setup.sh '${branch}'; claude --remote-control '${slug}'"
typed=0
for _ in 1 2 3; do
  sleep 2
